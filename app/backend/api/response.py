...
async def set_status(session: session_dep, data: SetStatus, current_response: Response = Depends(check_response), current_user: User = Depends(check_tenant)):
    
    await set_status_to_response(session, data, current_response, current_user)
