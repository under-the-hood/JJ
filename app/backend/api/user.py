...
async def edit_password(session: session_dep, data: EditPassword = Depends(validate_edit_password), current_user: User = Depends(check_user), redis: Redis = Depends(get_redis)):
