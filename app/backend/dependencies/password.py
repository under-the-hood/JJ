...
async def validate_edit_password(data: EditPassword, current_user: User = Depends(check_user)):
    verify_password(data.password, current_user.password)
    verify_passwords_match(data.password, data.repeat_new_password)

    return data


async def validate_delete_user(data: Delete = Depends(verify_password), current_user: User = Depends(check_user)):
    verify_password(data.password, current_user.password)
